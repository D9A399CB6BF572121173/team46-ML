"""
Created on Fri Nov 22 03:17:25 2019
...
data = data[~data['WORK'].str.contains('#NUM!')]
data.isnull().sum()
...
data['YEAR'].mean() #1979
data['SATIS'].mode()[0] #Average
data['SEX'].mode()[0] #Male
data['COUNT'].mode()[0] #Honduras
data['PROF'].mode()[0] #payment analyst
data['UNI'].mode()[0] #Bachelor
data['HAIR'].mode()[0] #Black
...
X_train, X_test = data[data['IsTrain']==1], data[data['IsTrain']==0]
...
#del_col = set(['TOTAL','Instance'])
#features_col =  list(set(X_train) - del_col)
#features_col =  list(set(X_test) - del_col)
...
## replace the below code with the algorithm you want to use
#params = {
#          'max_depth': 20,
#          'learning_rate': 0.01,
#          "boosting": "gbdt",
